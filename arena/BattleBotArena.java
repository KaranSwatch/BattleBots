...
import bots.Bot;
import bots.Drone;
import bots.HumanBot;
import bots.RandBot;
import bots.Rinnegan;
import bots.SentryBot;
import bots.SwatchBot;

/**
 * <b>Introduction</b> <br>
 * <br>
 * This class implements a multi-agent competitive game application. Players
 * contribute a single Java class that defines a "Bot", and these Bots battle
 * each other in a multiple round tournament under the control of a
 * BattleBotsArena object. For instructions on how to create a Bot, see the
 * documentation in the class named <i>Bot</i>. For instructions on how to add
 * Bots to the arena, see the documentation for the <i>fullReset()</i> method in
 * this class.<br>
 * <br>
 *
 * <b>The Game Engine</b><br>
 * <br>
 *
 * The Arena attempts to run at 30 frames per second, but the actual frame rate
 * may be lower on slower systems. At each frame, the arena does the following
 * for each Bot b that is still alive and not <i>overheated</i>:
 *
 * <ol>
 * <li>Gets the team name using <i>b.getTeamName()</i></li>
 *
 * <li>Gets the next move using <i>b.getMove(BotInfo, boolean, BotInfo[],
 * BotInfo[], Bullet[])</i></li>
 *
 * <li>Processes the move only if it is legal (i.e. moves are allowed only if no
 * collisions; bullets and messages are allowed only if the max number of
 * bullets is not exceeded)</li>
 *
 * <li>If the move was SEND_MESSAGE, calls <i>b.outGoingMessage()</i> to get the
 * message from the Bot, then broadcasts it to all live Bots using
 * <i>b.incomingMessage(int, msg)</i></li>
 *
 * <li>Draws each Bot using <i>b.draw(Graphics)</i></li>
 * </ol>
 * <br>
 * <b>Timing</b><br>
 * <br>
 *
 * The clock records real time in seconds regardless of the actual number of
 * frames that have been processed. The clock will run faster when the play is
 * sped up to x2, x4, or x8 (do this by mousing over the clock and using the
 * scroll wheel). However, depending on the speed of your computer, you may not
 * get 2x, 4x, or 8x as many frames in that time, so you should test your Bots
 * at regular speed whenever possible. <br>
 * <br>
 *
 * <b>Bot Movement</b> <br>
 * <br>
 ...
 * messaging is capped).<br>
 * <br>
 *
 * <b>Info Passed to the Bots</b><br>
 * <br>
 *
 * When asking for a Bot's move, the arena passes an array of Bullets, arrays of
 * info concerning live and dead Bots, a boolean indicating whether the Bot is
 * currently able to shoot, and a single object containing the public
 * information about the Bot itself (see the abstract class Bot for more info on
 * all this). No Bot is ever given any access to any internal variables or data
 * structures of the arena, the Bullets, or other Bots. <br>
 * <br>
 *
 * <b>Collisions</b> <br>
 * <br>
 *
 * The size of a Bot is defined by the constant <i>Bot.RADIUS</i>. The centre
 * point of each Bot is defined as <i>(x+Bot.Radius, y+Bot.RADIUS)</i>, where x
 * and y are the top left corner of a square in which the bot is inscribed. The
 * width and height of each Bot is <i>Bot.RADIUS * 2</i>. Each Bot has a
 * circular collision mask in a radius of Bot.RADIUS from this centre point.
 * Bullets have a single point collision mask (the pixel at the front of the
 * Bullet) and are created one pixel over from the edge of the Bot that fired
 * them, in the middle of the side from which they were fired (i.e. vertical
 * bullets have an x coordinate of <i>x+Bot.Radius</i> and horizontal bullets
 * have a y coordinate of <i>y+Bot.RADIUS</i>). Two bots have collided if the
 * euclidean distance between their centre points is equal to
 * <i>Bot.RADIUS*2</i> or less).<br>
 * <br>
 *
 * <b>Bot CPU Usage</b> <br>
 * <br>
 *
 * Processor time is monitored using calls to <i>System.nanoTime()</i> any time
 * a Bot method is used (i.e. for drawing, getting the next move, getting the
 * bot's name and team name, message processing, etc.) This is not perfect, but
 * it does give an approximate estimate of how much CPU each Bot is consuming. A
 * small number of points per round are awarded for low CPU usage (see
 * <i>Scoring</i> below). If the cumulative CPU time for any Bot exceeds 2
 * seconds (configurable using the static constants), the Bot will
 * <i>overheat</i> and become disabled. At this point, the Bot is replaced by a
 * stock image symbol and there will be no more method calls to that particular
 * Bot for the remainder of the round.<br>
 * <br>
 *
 * <b>Buggy Bots</b><br>
 * <br>
 ...
 * scoring penalty is applied (defined by static constants in this class).<br>
 * <br>
 *
 * <b>Dead Bots</b><br>
 * <br>
 *
 * When a Bot is destroyed, it is replaced with a "dead bot" icon and becomes an
 * obstacle on the course. There will be no more method calls to that particular
 * Bot for the remainder of the round.<br>
 * <br>
 *
 * <b>Scoring</b><br>
 * <br>
 *
 * Bots are awarded points for each kill (whether or not they are alive when
 * their bullet hits), and points for each second they stay alive in the round.
 * The round ends after a set time, or after there are 1 or fewer Bots left. If
 * a Bot is left at the end of the round, it is awarded time points based for
 * the entire length the round would have been if it had continued. Point values
 * are increased each each round.<br>
 * <br>
 *
 * In addition to these base points, there are penalties for each exception
 * thrown, and there is a small bonus for low CPU equal to two points minus the
 * number of seconds of CPU time used. This bonus allows ties to be broken (e.g.
 * if two Bots each kill four other Bots and survive to the end of the round, it
 * is usually possible to declare a winner based on CPU time). <br>
 * <br>
 *
 * Five Bots are dropped after each round, until six or fewer Bots are left. So
 * if the game starts with 16 Bots(the default) there will be 3 rounds of play.
 * (16 bots, 11 bots, and 6 bots respectively).<br>
 * <br>
 *
 * All scoring values and other numbers mentioned in this section can be
 * configured using the static constants of this class. In addition, the game
 * can be played so that the winning Bot is the one with the highest cumulative
 * score at the end of multiple rounds, or it can be the Bot that wins the final
 * round.<br>
 * <br>
 *
 * <b>Debugging Features</b><br>
 * <br>
 *
 * The arena contains a number of features to aid debugging. The screen that
 * comes up when the Arena is first run is a "Test Mode" screen that allows you
 * to track CPU time and exceptions, view the name and team of each Bot, and
 * check that drawing and movement is within the allowed limits. There is also a
 * DEBUG flag that can be set to TRUE to view statistics while the game is on.
 * The game can be sped up to make it easier to view the outcome of each test
 * match quickly (see the notes under <i>Timing</i> above), and pausing the game
 * provides just over one second of instant replay to watch your Bots' actions
 * in more detail. Finally, there is a "HumanBot" character that you can control
 * with the keyboard to further test your Bots' performance.<br>
 * <br>
 *
 *
 * @version 1.0 (March 3, 2011) - Initial Release
 * @version <br>
 *          1.1 (March 10, 2011) - Added correction factor for
 *          system.nanoTime(), fixed bug in messaging (was cutting off last
 *          character of every message)
 * @version <br>
 *          1.2 (March 24, 2011) - Added ready flag used in paint and
 *          paintBuffer to avoid exceptions from a race condition on startup
 * @version <br>
 *          1.3 (March 28, 2011) - Load starting team names at beginning of
 *          match, icons updated (thanks to Mike Stuart for the new dead bot
 *          icon)
 * @version <br>
 *          1.4 (March 28, 2011) - Improvements in how info is passed to bots:
 *          a. Only temp arrays are passed (so bots can't sabotage them); b. A
 *          deep copy of the BotInfo array for live bots is passed, so that all
 *          Bots get the exact same snapshot of where the Bots are (thanks to
 *          Zong Li for helping uncover the latter issue)
 * @version <br>
 *          1.5 (March 31, 2011) - Moved bullet processing out of the Bot loop
 *          -- now bots are moved first, then bullets are moved (thanks again to
 *          Zong Li for pointing out this issue)
 * @version <br>
 *          1.6 (May 30, 2011) - Shuts off sound on stop/destroy now
 * @version <br>
 *          1.7 (August 9, 2011) - Converted to an application that can be
 *          JAR'ed -- the mouse wheel was not working well when embedded in a
 *          web page
 * @version <br>
 *          1.8 (November 30, 2011) - Fixed audio bug *
 * @version <br>
 *          1.9 (May 8, 2015) - Rowbottom changed BOT_SPEED to 2 and made rounds
 *          30 sec and removal rate to 1 bot per round.
 * @version <br>
 *          1.9.1(May 2016) - Rowbottom changed functionality to allow running
 *          large number of rounds and adjusted scoring
 * @version <br>
 *          1.10 - Rowbottom thickened bullets to increase visibility
 * @version <br>
 *          1.11 (Nov 6 2017) - Rowbottom increased NUM_BOTS and screen size to
 *          large bounds and fixed hardcoded references
 * @version <br>
 *          1.12 (Nov 6.2017) - Rowbottom Increased bot radius to 13 and
 *          increased movement and bullet speed
 * @version <br>
 *          2.0 (November 15, 2015) - Rowbottom added limited ammo
 *          functionality. Bots start out with limited ammo and cannot fire if
 *          numBullets < 1
 * @version <br>
 *          2.1 (November 16, 2015) - Rowbottom extends limited ammo
 *          functionality so that liveBots can pickup used ammo off deadBots.
 * @version <br>
 *          2.2 (November 11, 2017) - Rowbottom carries fixes and tweaks made to
 *          BattleBots 1 to BattleBots 2.
 * 
 * 
 * @author Sam Scott
 *
 */
public class BattleBotArena extends JPanel
		implements MouseListener, MouseWheelListener, MouseMotionListener, ActionListener, Runnable {
	...
	/**
	 * For bot to request a message send. If allowed, the arena will respond with a
	 * call to the bot's outGoingMessage() method.
	 */
	 ...
	 */
	public static final int BOTTOM_EDGE = 896; // ROWBOTTOM - changed from 500// arena panel height is this constant +
												// TEXT_BUFFER
	/**
	...
	/**
	 * true = scores between rounds are cumulative false = highest scoring Bot in
	 * last round is declared the winner
	 */
	...
	/**
	 * Total number of Bots in round 1 (if you have fewer than this, the rest of the
	 * spots in the array will be filled with Drones, RandBots, and Sentries).
	 */
	public static final int NUM_BOTS = 20;// ROWBOTTOM NOV 6 2017 Changed to 25
	/**
	 ...
	 */
	public static final double BOT_SPEED = 3;// ROWBOTTOM changed from 2.5 to fix game balance and bots getting hung up
												// on tombstones
	/**
	...
	/**
	 * Initial ammo as part of limited ammo functionality !Passed to the botInfo for
	 * its value.
	 */
	public static final int BULLETS_LEFT = 20;// ROWBOTTOM Ammo

	/**
	 * When ELIMINATIONS_PER_ROUND is set to 0 then NUM_ROUNDS determines the final
	 * round
	 */
	...
	/**
	 * How many frames to hold on the last frame before restarting the instant
	 * replay.
	 */
	...
	/**
	 * The main state variable - controls what phase of the game we are at (see the
	 * State constants)
	 */
	private int state = WAIT_TO_START; // state variable
	/**
	 * Used when going into test mode - false while bots are being set up,
	 * constructors called, etc.
	 */
	...
	/**
	 * On some machines, System.nanoTime() returns incorrect results. For example,
	 * on one machine System.currentTimeMillis() shows 10 seconds elapsed while
	 * System.nanoTime() consistently shows 4.5 seconds elapsed for the same time
	 * period. The more reliable millisecond timing is used for the game clock,
	 * however for timing CPU usage of the Bots, we need a higher grain than 1ms. So
	 * System.nanoTime() is used, but a correction factor is computed in a thread
	 * spawned at startup time, and this becomes a multiplier for the number of ns
	 * that System.nanoTime() reports has elapsed.
	 */
	...
	/**
	 * Main array of Bot objects. Note that bots, botsInfo, and bullets are related
	 * arrays - bullets[i] gives the array of bullets owned by bot[i], and
	 * botsInfo[i] gives the public info for bot[i].
	 */
	private Bot[] bots = new Bot[NUM_BOTS];
	/**
	 * Array of public info regarding the Bots - this is how information is passed
	 * to the Bots in getMove(). Done this way so the Bots don't have access to each
	 * others' internal states. Note that bots, botsInfo, and bullets are related
	 * arrays - bullets[i] gives the array of bullets owned by bot[i], and
	 * botsInfo[i] gives the public info for bot[i].
	 */
	private BotInfo[] botsInfo = new BotInfo[NUM_BOTS];
	/**
	 * The bullets. Note that bots, botsInfo, and bullets are related arrays -
	 * bullets[i] gives the array of bullets owned by bot[i], and botsInfo[i] gives
	 * the public info for bot[i].
	 */
	...
	/**
	 * The latest frame. The head pointer of the circular buffer called
	 * replayImages.
	 */
	...
	/**
	 * This is a buffer that holds the images the Bots are requesting to load. Since
	 * bots don't have access to the arena as an image observer, the images in this
	 * list are painted off screen until g.drawImage() returns true. This ensures
	 * that all images get loaded ASAP whether the Bots are using them or not, and
	 * that callbacks happen and trigger a repaint when images are loaded.
	 */
	 ...
	 *
	 * NOTE: This is where you add your own bots. See the instructions in the method
	 * below...
	 */
	private void fullReset() {
		ready = false; // Signals to the paint methods that the Bots are not set up yet
		if (soundOn)
			open.play(); // Play the fanfare
		...
		bots[2] = new HumanBot();
		addKeyListener((HumanBot) bots[2]);
		// ******************************
		...
		bots[1] = new SwatchBot();
		bots[0] = new SentryBot();
		bots[3] = new RandBot();
		//bots[4] = new Rinnegan();
...
//				}
	...
	public BattleBotArena() {
		...
		overheated = Toolkit.getDefaultToolkit()
				.getImage(getClass().getClassLoader().getResource("images/overheated.png"));
	...
	private void reset() {
		...
		// Clear the array of public Bot info. (This is the info given to the Bots when
		// making their moves.)
		...
		{
			int xScale = (RIGHT_EDGE - Bot.RADIUS * 4) / Math.max(NUM_BOTS - 1, 1); // this spaces them out so they
																					// don't rez on top of each other
			...
			for (int i = 0; i < NUM_BOTS; i++) {
				...
				newBotsInfo[i] = new BotInfo(x * xScale + Bot.RADIUS, y * yScale + Bot.RADIUS, i, bots[i].getName()); // create
																														// new
																														// BotInfo
																														// object
																														// to
			...
			for (int i = 0; i < NUM_BOTS; i++) {
				...
				x += offsetx;
				y += offsety;
				// System.out.println(bots[i].getName()+ " "+newBotsInfo[i].getName()+"
				// "+newBotsInfo[i].isOut());
			}
		}
			...
			for (int i = 0; i < imagePaths.length; i++) {
				try {
					images[i] = Toolkit.getDefaultToolkit()
							.getImage(getClass().getClassLoader().getResource("images/" + imagePaths[i]));
	...
	/**
	 * This method is for the thread computes the correction factor for
	 * System.nanoTime(). See the documentation of the field "nanoTimeCorrection"
	 * for more information. The thread is spawned by the init() method, and should
	 * be complete and no longer running after about 20s.
	 */
	public void run() {
		// pause for setup
		try {
	...
	/**
	 * The main game method - called by the timer. Handles all the mechanics of the
	 * game, the replay mode, and the test mode screen.
	 */
					...
					if (!botsInfo[j].isOut())
						if (!botsInfo[j].isDead())
							...
							deadBots[nextDeadBotIndex++] = botsInfo[j].copy(); // important to deep copy or else some
																				// bots will get info about the current
																				// move
				...
				for (int i = 0; i < NUM_BOTS; i++) {
					...
					if (!botsInfo[i].isOverheated() && !botsInfo[i].isDead() && !botsInfo[i].isOut()) {
						...
						} else // bot still alive! Process move
						{
							...
							startThink = System.nanoTime();
							try {
								move = bots[i].getMove(currentBot, shotOK, cleanLiveBotsArray, cleanDeadBotsArray,
										cleanBulletArray);
							} catch (Exception e) {
								botsInfo[i].exceptionThrown(e);
							}
							...
							case FIREUP:
								for (int j = 0; j < NUM_BULLETS; j++) // looks for the first unused bullet slot
									if (bullets[i][j] == null && botsInfo[i].getBulletsLeft() > 0) {
										bullets[i][j] = new Bullet(botsInfo[i].getX() + Bot.RADIUS,
												botsInfo[i].getY() - 1, 0, -BULLET_SPEED);
										if (state != TEST_MODE) {
											botsInfo[i].setBulletsLeft(botsInfo[i].getBulletsLeft() - 1);// reduce the
																											// amount of
																											// bullets
																											// by one.
											if (soundOn)
							...
							case FIREDOWN:
								for (int j = 0; j < NUM_BULLETS; j++)// looks for the first unused bullet slot
									if (bullets[i][j] == null && botsInfo[i].getBulletsLeft() > 0) {
										bullets[i][j] = new Bullet(botsInfo[i].getX() + Bot.RADIUS,
												botsInfo[i].getY() + Bot.RADIUS * 2 + 1, 0, BULLET_SPEED);
										if (state != TEST_MODE) {
											botsInfo[i].setBulletsLeft(botsInfo[i].getBulletsLeft() - 1);// reduce the
																											// amount of
																											// bullets
							...
							case FIRELEFT:
								for (int j = 0; j < NUM_BULLETS; j++)// looks for the first unused bullet slot
									if (bullets[i][j] == null && botsInfo[i].getBulletsLeft() > 0) {
										bullets[i][j] = new Bullet(botsInfo[i].getX() - 1,
												botsInfo[i].getY() + Bot.RADIUS, -BULLET_SPEED, 0);
										if (state != TEST_MODE) {
											botsInfo[i].setBulletsLeft(botsInfo[i].getBulletsLeft() - 1);// reduce the
																											// amount of
																											// bullets
							...
							case FIRERIGHT:
								for (int j = 0; j < NUM_BULLETS; j++)// looks for the first unused bullet slot
									if (bullets[i][j] == null && botsInfo[i].getBulletsLeft() > 0) {
										bullets[i][j] = new Bullet(botsInfo[i].getX() + Bot.RADIUS * 2 + 1,
												botsInfo[i].getY() + Bot.RADIUS, BULLET_SPEED, 0);
										if (state != TEST_MODE) {
											botsInfo[i].setBulletsLeft(botsInfo[i].getBulletsLeft() - 1);// reduce the
																											// amount of
							...
							if (move == UP || move == DOWN || move == LEFT || move == RIGHT) // if a move was made...
							{
								...
								for (int j = 0; j < NUM_BOTS; j++) {
									...
									{
										double d = Math.sqrt(Math.pow(botsInfo[i].getX() - botsInfo[j].getX(), 2)
												+ Math.pow(botsInfo[i].getY() - botsInfo[j].getY(), 2));
										if (d < Bot.RADIUS * 2) {
											// ROWBOTTOM handles the looting from dead bodies
											if (botsInfo[j].getBulletsLeft() > 0 && botsInfo[j].isDead()) {
												botsInfo[i].setBulletsLeft(
														botsInfo[i].getBulletsLeft() + botsInfo[j].getBulletsLeft());// live
																														// bot
																														// gets
																														// the
																														// bullets
									...
									for (int j = 0; j < NUM_BOTS; j++) {
										if (!botsInfo[j].isOut() && i != j) {
											double d = Math.sqrt(Math
													.pow(bullets[i][k].getX() - (botsInfo[j].getX() + Bot.RADIUS), 2)
													+ Math.pow(bullets[i][k].getY() - (botsInfo[j].getY() + Bot.RADIUS),
															2));
											if (d < Bot.RADIUS) // hit something
											{
												...
												if (botsInfo[j].isDead() == false) // kill bot if possible
												{
													...
													botsInfo[j].setScore(currentScore(j, false)); // final score of dead
																									// bot
													// botsInfo[i].setScore(currentScore(i,false));
													botsLeft--;
													sendMessage(SYSTEM_MSG, botsInfo[j].getName() + " destroyed by "
															+ botsInfo[i].getName() + ".");
												}
	 ...
	 * @param gameOver Whether or not the game is over (full score alloted in this
	 *                 case)
	 * @return The score
	 */
	private double currentScore(int botNum, boolean gameOver) {
		...
		if (ELIMINATIONS_PER_ROUND == 0) {
			score = KILL_SCORE * botsInfo[botNum].getNumKills() - ERROR_PENALTY * botsInfo[botNum].getNumExceptions()
					+ EFFICIENCY_BONUS * (PROCESSOR_LIMIT - botsInfo[botNum].getThinkTime());
		} else {
			score = KILL_SCORE * botsInfo[botNum].getNumKills() * (round + 1.0) / 2
					- ERROR_PENALTY * botsInfo[botNum].getNumExceptions()
					+ EFFICIENCY_BONUS * (PROCESSOR_LIMIT - botsInfo[botNum].getThinkTime());
		}
		...
		if (gameOver)
			...
			else if (ELIMINATIONS_PER_ROUND == 0) {
	...
	private BotInfo[] sortedBotInfoArray(boolean descending) {
		...
		for (int i = NUM_BOTS - 1; i > 0; i--)
			for (int j = 1; j <= i; j++) {
				...
				if (descending && score1 > score2 || !descending && score1 < score2
						|| descending && score1 == score2 && !(newInfos[j].isOut() || newInfos[j].isOutNextRound())
								&& (newInfos[j - 1].isOut() || newInfos[j - 1].isOutNextRound())) {
	...
	/**
	 * This method paints the bots and bullets for the game area into the instant
	 * replay buffer. Then it calls a repaint to trigger a call to the paint method
	 * which displays this buffer to the screen.
	 */
			...
			for (int i = 0; i < NUM_BOTS; i++) {
				if (!botsInfo[i].isOut()) // skip bots that are out
					if (botsInfo[i].isDead()) // dead bot
						g.drawImage(deadBot, (int) (botsInfo[i].getX() + 0.5), (int) (botsInfo[i].getY() + 0.5),
								Bot.RADIUS * 2, Bot.RADIUS * 2, this);
					else if (botsInfo[i].isOverheated()) // overheated bot
						g.drawImage(overheated, (int) (botsInfo[i].getX() + 0.5), (int) (botsInfo[i].getY() + 0.5),
								Bot.RADIUS * 2, Bot.RADIUS * 2, this);
					else // active bot
					{
						// BOT METHOD CALL - timed and exceptions caught
						...
						if (state == TEST_MODE) {
							if (flashCounter < FLASH_TIME / 2) {
								...
								g.drawOval((int) (botsInfo[i].getX() + 0.5) - 1, (int) (botsInfo[i].getY() + 0.5) - 1,
										Bot.RADIUS * 2 + 2, Bot.RADIUS * 2 + 2);
							}
							if (state == TEST_MODE) {
								g.setFont(new Font("MonoSpaced", Font.PLAIN, NAME_FONT));
								g.setColor(Color.gray);
								g.drawString("" + botsInfo[i].getNumExceptions(),
										(int) (botsInfo[i].getX() + 0.5) + Bot.RADIUS * 2 + 2,
										(int) (botsInfo[i].getY() + 0.5) + Bot.RADIUS);
								g.drawString("" + df2.format(botsInfo[i].getThinkTime()),
										(int) (botsInfo[i].getX() + 0.5) + Bot.RADIUS * 2 + 2,
										(int) (botsInfo[i].getY() + 0.5) + Bot.RADIUS + NAME_FONT);
							}
						}
					}
			...
			for (int i = 0; i < NUM_BOTS; i++) {
				if (botsInfo[i].isDead() == false && botsInfo[i].isOut() == false) {
					...
					if (showNames)
						...
						// x calculation based on x-width of 0.5 font size with a one pixel spacer
						// between letters
						g.drawString(
								title, (int) (botsInfo[i].getX() + Bot.RADIUS
										- (title.length() / 2.0 * (NAME_FONT * 0.5 + 1)) + 0.5),
								(int) (botsInfo[i].getY() - 1 + 0.5));
				}
			}
	...
	/**
	 * This method prints out the stats for each robot in sorted order. Used at the
	 * end of each round, and also during the game when the DEBUG flag is set.
	 * 
	...
	private void printStats(Graphics g) {
		...
		if (state != WAIT_TO_START) {
			...
			g.drawString("Name     Team     Round  Total  Time  Errors  Messages  Processor  Killed By", xOffset,
					yOffset);
			for (int i = 0; i < NUM_BOTS; i++) {
				String output = pad(newInfos[i].getName(), 8, false) + " " + pad(newInfos[i].getTeamName(), 8, false)
						+ " ";
				output += (newInfos[i].isOut() ? "     " : pad(df.format(newInfos[i].getScore()), 5, true)) + "  "
						+ pad(df.format(newInfos[i].getScore() + newInfos[i].getCumulativeScore()), 5, true) + " ";
				if (!newInfos[i].isOut()) {
					output += (newInfos[i].isDead() ? pad(df.format(newInfos[i].getTimeOfDeath()), 5, true)
							: (state == GAME_OVER || state == WINNER ? pad(df.format(TIME_LIMIT), 5, true)
									: pad(df.format(timePlayed), 5, true)))
							+ " ";
					output += pad("" + newInfos[i].getNumExceptions(), 6, true) + "    "
							+ pad("" + newInfos[i].getNumMessages(), 4, true) + "    ";
					output += pad(df2.format(newInfos[i].getThinkTime()), 8, true) + "    "
							+ pad(newInfos[i].getKilledBy(), 8, false);
				}

				if (newInfos[i].isDead() && state != GAME_OVER && state != WINNER || newInfos[i].isOut()
						|| state == GAME_OVER && newInfos[i].isOutNextRound() || state == WINNER && i != 0)
					g.setColor(Color.gray);
				else
	...
	/**
	 * Paints the screen. Assumes that paintBuffer() has been called recently to
	 * paint the current game state into the instant replay buffer.
	 * 
		...
		if (ready) // avoid race condition on startup
		{
			...
			if (state == GAME_PAUSED) {
				...
				if (CUMULATIVE_SCORING)
					g.drawString(botsInfo[winnerID].getName() + " Leading After " + round + " Round"
							+ (round > 1 ? "s" : "") + ". Click This Bar to Continue.", 10, BOTTOM_EDGE - 10);
				else
					g.drawString(
							botsInfo[winnerID].getName() + " Wins Round " + round + ". Click This Bar to Continue.", 10,
							BOTTOM_EDGE - 10);
				printStats(g);
				...
				g.setFont(new Font("MonoSpaced", Font.BOLD, 20));
				if (CUMULATIVE_SCORING)
					g.drawString(botsInfo[winnerID].getName() + " Wins after " + round + " Round"
							+ (round > 1 ? "s" : "") + "! Click This Bar to Restart.", 10, BOTTOM_EDGE - 10);
				else
					g.drawString(botsInfo[winnerID].getName() + " Wins the Final Round! Click This Bar to Restart.", 10,
							BOTTOM_EDGE - 10);
				printStats(g);
				...
				y += 15;
				g.drawString("- each robot can send " + (int) (TIME_LIMIT / SECS_PER_MSG) + " messages per round", 10,
						y);
				y += 15;
				...
				y += 16;
				if (ELIMINATIONS_PER_ROUND == 0) {
					g.drawString("- " + df.format(KILL_SCORE) + " points per kill, " + df.format(POINTS_PER_SECOND * 10)
							+ " points per 10 seconds of survival", 10, y);// round
				} else {
					g.drawString("- " + df.format(KILL_SCORE * (round + 1.0) / 2) + " points per kill, "
							+ df.format(POINTS_PER_SECOND * (round + 1.0) / 2 * 10)
							+ " points per 10 seconds of survival", 10, y);
				}
				y += 15;
			...
			if (state != GAME_PAUSED || pauseCount < PAUSE_FLASH_TIME / 2) {
				...
				g.drawString("" + pad(df.format(Math.abs(TIME_LIMIT - timePlayed)), 5, true), RIGHT_EDGE - 152,
						BOTTOM_EDGE + 40);
	...
	/**
	 * Handles user's mouse clicks on the menu buttons, the time clock, and the
	 * "click here" bars.
	 * 
	...
	public void mousePressed(MouseEvent e) {
		if (e.getButton() == MouseEvent.BUTTON1) // left button only
		{
			...
			} else if (e.getX() >= RIGHT_EDGE - 125 && e.getX() <= RIGHT_EDGE - 125 + 49)// clicked on names or teams
																							// button
			{
			...
			} else if (e.getX() >= RIGHT_EDGE - 74 && e.getX() <= RIGHT_EDGE - 69 + 55)// clicked on sound or scores
																						// button
			{
				if (e.getY() >= BOTTOM_EDGE + 56)
	...
	/**
	 * Scroll event. Scroll the messages or change game speed depending on location
	 * of the mouse.
	 * 
